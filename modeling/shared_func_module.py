...
def temp_get(input_ids, ori_text_vocab_size, vision_start_id) -> torch.Tensor:
    """根据 input_ids 确定哪些 token 属于视觉模态。"""
    # 这个函数必须被 monkey patch
    raise NotImplementedError


def get_ordinary_vision_mask(input_ids, ori_text_vocab_size, vision_start_id) -> torch.Tensor:
...
# Future-TODO 这个最好还是设置为 temp_get，但是现在为了推理代码比较方便，先暂时这样设置。
get_vision_mask = get_ordinary_vision_mask


def monkey_patch_get_vision_mask(data_args, tokenizer):
    global get_vision_mask
    # Configure tokenizer and conversation template
    if data_args.data_format == 'default':
        ...
        if data_args.add_sep_for_vis:
            ...
            get_vision_mask = shared_func_module.reset_get_vis_mask(vis_sep_tokens_id)
        else:
            get_vision_mask = shared_func_module.get_ordinary_vision_mask
    elif data_args.data_format == 'emu3':
        # Future-TODO 完成emu3任意图片分辨率适配
        pass
    else:
        raise ValueError(f'Not supported data format {data_args.data_format}')
