...
from modeling.shared_func_module import get_flash_attention_args_tensorized, UniQwen3Config
...
class UniQwen3ModelInference(Qwen3Model):
    ...
    def forward(
        ...
        _tmp_ffn_vis_mask = shared_func_module.get_vision_mask(input_ids, self.config.real_text_vocab_size, self.config.vision_start_id)
        # set vis_mask buffer
        for n, mod in self.named_modules():
            if isinstance(mod, UniShareQwen3MLP):
