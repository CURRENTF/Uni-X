...
def parse_args():
    ...
    parser.add_argument(
        "--csv",
        type=str,
        default='evaluation/T2I_Eval/dpg_bench/dpg_bench.csv',
    )
    parser.add_argument(
        "--res-path",
        type=str,
...
# 这个模型只支持batch_size=1，能否通过使用多线程或者多进程大幅度提高测试速度？
# 多线程是因为感觉只需要给GPU队列提交任务就可以了，不需要占用很多CPU计算？
class MPLUG(torch.nn.Module):
    def __init__(self, ckpt='damo/mplug_visual-question-answering_coco_large_en', device='gpu', num_workers=4):
        super().__init__()
        from modelscope.pipelines import pipeline
        from modelscope.utils.constant import Tasks
        self.pipeline_vqa = pipeline(Tasks.visual_question_answering, model=ckpt, device=device)
        # 使用线程池并行处理VQA请求，显著提高对单图多问题场景的处理速度
    ...
    def vqa_batch(self, image, questions):
        """
        使用线程池并行处理单个图像的多个问题。

        Args:
            image: PIL.Image, 输入图片。
            questions: list, 一个包含 (id, question_text) 元组的列表。

        Returns:
            dict, 一个将 question id 映射到 answer 的字典。
        """
        # 提交所有VQA任务到线程池
        future_to_qid = {self.executor.submit(self.vqa, image, q_text): q_id for q_id, q_text in questions}
        ...
        for future in concurrent.futures.as_completed(future_to_qid):
            ...
            try:
                qid_to_answer[qid] = future.result()
            except Exception as e:
                print(f"处理问题ID {qid} 时出错: {e}")
                qid_to_answer[qid] = "error"
...
def compute_dpg_one_sample(args, question_dict, image_path, vqa_model, resolution):
    ...
    for crop_tuple in crop_tuples:
        ...
        # 为提高效率，批量处理当前裁剪图像的所有问题
        questions_to_ask = list(qid2question.items())
        qid2answer = vqa_model.vqa_batch(cropped_image, questions_to_ask)

        qid2scores = {}
        with open(args.res_path.replace('.txt', '_detail.txt'), 'a') as f:
            # 按问题ID排序以保持输出一致性
            for qid in sorted(qid2answer.keys()):
...
def main():
    ...
    if args.vqa_model == 'mplug':
        vqa_model = MPLUG(ckpt="../mock/models/mplug_dpg", device=device)
    else:
