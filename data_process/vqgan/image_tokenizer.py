...
from data_process.vqgan.vqgan import VQModel
...
class ImageTokenizer:
    ...
    def img_tokens_from_pil(self, image: PIL.Image) -> list[int]:
        ...
        img = self._vqgan_input_from(img).to(self._device).to(self._dtype)
        #### if you want to support any resolution
        # np_img = np.array(img) / 255.0  # Normalize to [0, 1]
        # np_img = np_img * 2 - 1  # Scale to [-1, 1]
    ...
    def img_tokens_from_pil_batch(self, images: list[PIL.Image], target_image_size: int = 512) -> list[list[int]]:
        """
        Tokenizes a batch of PIL images into VQGAN image tokens.
        !!! 还没测试过

        Args:
            images (list[PIL.Image]): A list of PIL images to be tokenized.
            target_image_size (int): The target size to which images will be resized and cropped.

        Returns:
            list[list[int]]: A list of token lists, where each inner list corresponds
                             to an input image.
        """
        if not images:
            return []

        # Process each image and collect the resulting tensors.
        # This pre-processing ensures all images are the same size for batching.
        processed_tensors = [
            self._vqgan_input_from(self._whiten_transparency(img), target_image_size)
            for img in images
        ]

        # Stack the individual tensors into a single batch tensor.
        image_batch = torch.cat(processed_tensors, dim=0)
        image_batch = image_batch.to(device=self._device, dtype=self._dtype)

        # Encode the entire batch at once for efficiency.
        with torch.no_grad():
            _, _, [_, _, vqcode_batch] = self._vq_model.encode(image_batch)

        # Reshape the output to have one list of tokens per image.
        batch_size = image_batch.shape[0]
        vqcode_batch = vqcode_batch.view(batch_size, -1)

        # Return the tokens as a standard Python list of lists.
        return vqcode_batch.cpu().tolist()